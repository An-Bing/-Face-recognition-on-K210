...
void Add_FR(void)
{
  ...
  while(1)
  {
    switch (processnum)
    {
			...
			case 4:
				OLED_ShowCH(0, 0, " 按ADD加,按DEC减 ");
				OLED_ShowCH(0, 2, "    按SAVE保存    ");
				OLED_ShowCH(0, 4, "  0=< ID <=99   ");
				while(key_num != KEY_SAVE)
				{
					key_num = key_scan(1);
					if(key_num == KEY_DEC)
					{
...
//          DEBUG_LOG(str, " ID:%d 得分:%d ", seach.pageID, seach.mathscore);
//          OLED_ShowCH(0, 4, (u8*)str);
					FingerFlag = FingerRight;
          delay_ms(1500);
          delay_ms(1500);
					break;
        }
...
void Del_FR(void)
{
  ...
  u16 ID_NUM = 0;
	OLED_Clear();
  OLED_ShowCH(0, 2, "  ADD加  DEC减 ");
  OLED_ShowCH(0, 4, "    SAVE确认  ");
  OLED_ShowCH(0, 0, "   删除 指纹 ");
  while(key_num != KEY_SAVE)
  {
		key_num = key_scan(1);
    if(key_num == KEY_DEC)
