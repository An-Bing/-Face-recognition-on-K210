	...
	while (*chs != '\0') 
	{
		if (*chs > 0xa0) 
		{
			for (i = 0; i < HZ; i++) 
			{
				...
				if ((*chs == hz_index[i * 2]) && (*(chs + 1) == hz_index[i * 2 + 1])) 
				{
					OLED_Set_Pos(x, y);
					for (j = 0; j < 16; j++)
					OLED_WR_Byte(Hzk[i * 2][j],OLED_DATA);
					OLED_Set_Pos(x, y + 1);
					for (j = 0; j < 16; j++)
					OLED_WR_Byte(Hzk[i * 2 + 1][j],OLED_DATA);
					x += 16;
					break;
				}
			}
		...
		else 
		{
			...
			if (m != NULL)
			{
				OLED_Set_Pos(x, y);
				for (j = 0; j < 8; j++)
					OLED_WR_Byte(F8X16[((u8)*m - ' ') * 16 + j],OLED_DATA);
				OLED_Set_Pos(x, y + 1);
				for (j = 0; j < 8; j++)
					OLED_WR_Byte(F8X16[((u8)*m - ' ') * 16 + j + 8],OLED_DATA);
				x += 8;
			}
			chs++;
		}
...
{      			    
    ...
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(Hzk2[no][t],OLED_DATA);
				adder+=1;
     }	
}
